<?php
/*
Plugin Name: AeSports.co.uk - Tournament Bracket System
Plugin URI: http://www.aesports.co.uk
Description: Plugin for tournament brackets.
Author: DrowninG
Version: 1.0 BETA
Author URI: http://www.aesports.co.uk
*/
...
<h2>AE Sports - Tournament Bracket System</h2>
<br />
<br />
Welcome to AE's tournament bracket system.<br />
...
/**Get all the data from the tabe wp_bracket_8player**/
function bracket_get_tournament_8p_de() {
    global $wpdb;
    $tournament_name = $wpdb->get_results("SELECT * FROM ".$wpdb->prefix."bracket_8player_de ORDER BY id ASC");
    return $tournament_name;
}

require_once('bracket_8p_de.php');

    bracket_action();
    if (empty($_GET['edit'])) {
      /**Display the data into the Dashboard**/
        bracket_manage_tournament_8p_de();
    } else {
      /**Display a form to add or update the data**/
        bracket_add_tournament_8p_de();   
    }
}
...
/**Get all the data from the tabe wp_bracket_16player**/
function bracket_get_tournament_16p_de() {
    global $wpdb;
    $tournament_name = $wpdb->get_results("SELECT * FROM ".$wpdb->prefix."bracket_16player_de ORDER BY id ASC");
    return $tournament_name;
}
require_once('bracket_16p_de.php');

    bracket_action();
    if (empty($_GET['edit'])) {
      /**Display the data into the Dashboard**/
        bracket_manage_tournament_16p_de();
    } else {
      /**Display a form to add or update the data**/
        bracket_add_tournament_16p_de();   
    }
}
